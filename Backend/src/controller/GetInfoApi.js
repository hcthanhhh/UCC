...
exports.GetResultUCC = async (req, res) => {
    request = req.body;
    username = request.username;
    name = request.name;
    ...
    result = [];
    try {
        _CheckFile = await CheckFileAvailable(username, name, 'TOTAL_outfile.csv');
        console.log("wassup");
...
exports.GetSLOCandType = async (req, res) => {
    request = req.body;
    username = request.username;
    name = request.name;
    ...
    try {
        _CheckFile = await CheckFileAvailable(username, name, 'outfile_summary.csv');
        fs.createReadStream(`../data/result/${username}/${name}/outfile_summary.csv`)
            .pipe(csv())
            .on('error', (err) => res.status(403).send({ message: 'Error' }))
            .on('data', row => {
                if (check) {
...
function GetCyclomaticResultFormatted(username, name) {
    return new Promise(async (resolve, reject) => {
        ...
        countFunction = 0;
        try {
            _CheckFile = await CheckFileAvailable(username, name, 'outfile_cyclomatic_cplx.csv');
